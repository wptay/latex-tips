...
% Starred version for floats like figures and tables without a frame.
\DeclareDocumentCommand{\revision}{ s m o o }{%
  \IfBooleanTF{#1}{%
    \IfValueT{#3}{%
      \xrcounterref{#3}{#4}
    }
    \ExecuteMetaData[\maintext]{#2}%%
  }{%
    \revisionframe{#2}%
  }%
}
...
\xrcounterref{Theorem}{thm:limit_var}
\revision{tag:thm}
...
\end{comment}
\begin{response}
Floats like tables and figures are reproduced using the starred version of the \verb|\revision| command. For example, the following figure and tables from the main text are reproduced here:
